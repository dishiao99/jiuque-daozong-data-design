...
| demo 指标 | 一期口径 | 数据源 | 计算方式 |
| --- | --- | --- | --- |
| 营业额 | 默认核销金额，另保留成交 GMV | 美团交易、抖音交易 | `sum(核销金额)`；GMV 用 `sum(下单金额 / 意向成交金额)` |
| 订单量 | 默认核销券数，另保留下单券数 | 美团交易、抖音交易 | `sum(核销券数)`；下单口径用 `sum(下单券数 / 意向成交券数)` |
| 单均 | 核销营业额 / 核销券数 | 美团交易、抖音交易 | 查询层派生 |
| 总好评数 | 新增好评数 + 明细星级高分聚合 | 美团评价汇总、评价明细、抖音评价 | 美团优先用汇总；抖音用 `推荐等级 >= 4星` |
| 单包间产值 | 一期不可由平台 Excel 得到 | 二期小程序/POS/房型 | `营业额 / 可售包间数` |
| 翻台率 | 一期不可由平台 Excel 得到 | 二期订单/房态 | `场次数 / 可售包间数` |
| 私域好友数 | 一期不可由平台 Excel 得到 | 二期企微/小程序 | 外部累计值 |

### 3.2 平台数据卡片

| demo 区块 | 美团/点评来源 | 抖音来源 | 备注 |
| --- | --- | --- | --- |
| 流量数据 | 客流数据：曝光人数、访问人数、意向转化人数、下单人数 | 商品报表：商品曝光/访问；交易报表：意向成交人数 | 抖音商品可按日导出，但当前商品报表缺门店维度 |
| 营业数据 | 商品交易数据：下单金额、核销金额、退款金额 | 交易报表：意向成交、核销、退款 | dashboard 默认核销收入 |
| 评价数据 | 评价数据、门店评价详情 | 门店评价明细 | 明细用于 AI 话术和差评分析 |
| 榜单排名 | 榜单数据 | 暂无 | 抖音没有本批榜单样本 |
| 经营评分/ROS | 本批样本未提供 | 本批样本未提供 | 需要另找导出入口或人工维护 |
...
| 标记 | demo 模块/字段 | 一期数据来源 | 口径与计算 | 需要客户确认 |
| --- | --- | --- | --- | --- |
| A1 | 业务部、区域、门店筛选 | 新增运营组织维表：`store_business_departments`、`store_operation_regions`、`store_region_assignments` | 先按 `business_vertical='general'` 限定到综门店，再按业务部、区域得到门店集合，最后过滤所有指标 | 久雀内部业务部/区域清单、门店归属、生效日期 |
| A2 | 日期、工作日/周末、同比、环比 | 日期来自各 Excel 的 `日期`；工作日/周末由日历派生 | 日期范围用于事实表过滤；同比/环比在查询层计算，不来自 Excel | 是否需要节假日口径；环比是上一周期还是上周同期 |
| A3 | 营业额 | 美团交易 `核销金额`；抖音交易 `核销金额`；小程序/POS 待补 | 默认 `sum(核销金额)`，GMV 另保留 `下单金额 / 意向成交金额` | demo 展示的是实收/核销，还是成交 GMV |
| A4 | 订单量 | 美团交易 `核销券数`；抖音交易 `核销券数`；小程序/POS 待补 | 默认 `sum(核销券数)`；下单口径另保留 `下单券数 / 意向成交券数` | “订单量”按券数、订单数还是用户数 |
| A5 | 单均 | 美团/抖音交易派生 | `营业额 / 订单量`，默认按核销口径 | 分母是否按核销券数 |
| A6 | 单包间产值 | 当前 9 个平台 Excel 不提供 | 二期需要房型/包间数量、小程序或 POS 订单 | 可售包间数、营业包间数、实际开台数用哪个做分母 |
| A7 | 翻台率 | 当前 9 个平台 Excel 不提供 | 二期需要包间/场次/订单时段数据 | 是否按包间、房型、门店营业时长计算 |
| A8 | 私域好友数 | 当前 9 个平台 Excel 不提供 | 二期接企微/小程序会员数据 | 私域好友是累计好友、净增好友还是可触达用户 |
| A9 | 总好评数 | 美团评价汇总 `新增好评数`；抖音评价明细 `推荐等级 >= 4星` | 按门店、日期、平台汇总；美团/点评从汇总表优先，抖音从评价明细计算 | 抖音 `4星` 是否算好评；中评/差评分界 |
...
| 标记 | demo 模块/字段 | 一期数据来源 | 口径与计算 | 需要客户确认 |
| --- | --- | --- | --- | --- |
| B1 | 大众点评/美团：曝光人数、访问人数、下单人数 | 美团客流数据：`曝光人数`、`访问人数`、`下单人数` | 按 `日期 x 门店 x 平台` 汇总；如 Excel 平台列为“点评/美团”，分别写 `dianping` / `meituan` | demo 卡片是否合并展示点评+美团，还是保留平台切换 |
| B2 | 大众点评/美团：牌级、经营评分 | 当前 9 个 Excel 不提供 | 暂标为待接入字段；可人工维护或后续补导出入口 | 牌级和经营评分是否必须一期上线 |
| B3 | 大众点评/美团：评价好/中/差、好评率、星级评分 | 美团评价汇总、门店评价详情 | 好评/差评优先用汇总；中评用 `新增评价数 - 新增好评数 - 新增差评数` 或明细星级计算；星级评分从明细均值或外部评分补充 | 中评计算方式；星级评分是否接受明细均值 |
| B4 | 大众点评/美团：榜单排名 | 美团榜单数据：`城市排名`、`行政区排名`、`商圈排名` | demo 默认展示商圈排名；缺失时可降级展示行政区/城市 | 优先展示商圈、行政区还是城市榜 |
| B5 | 大众点评/美团：营业额、订单量、单均 | 美团商品交易数据 | 营业额默认 `核销金额`，订单量默认 `核销券数`，单均派生 | 核销金额和下单金额的展示取舍 |
| B6 | 抖音：访问人数、下单人数 | 抖音商品报表 `商品访问人数`；抖音交易报表 `意向成交人数` / `核销人数` | 交易建议用按日交易报表；商品流量如需单店，需要重新导出带门店维度样本 | 下单人数取意向成交人数还是核销人数 |
| B7 | 抖音：货架点击人数 | 当前样本没有明确同名字段 | 候选：`商品访问人数` 或 `商品访问人数(不含商品卡)`，需确认后固定 | “货架点击”在抖音后台对应哪个指标 |
| B8 | 抖音：经营分、评分、评价数 | 评价数来自抖音评价明细 count；经营分本批无字段；评分可用 `推荐等级` 均值弱口径 | 推荐等级均值只能近似评分，不等于平台门店评分 | 经营分导出入口；评分是否接受近似 |
| B9 | 抖音：门店页流量来源 | 当前交易/商品/评价样本不提供来源占比 | 需要补抖音门店流量来源导出或 API | 推荐/搜索/同城/其他是否必须一期上线 |
| B10 | 抖音：成交分析、营业额、订单量、单均 | 抖音交易报表 | 成交分析可用 `意向成交金额/券数/人数`；营业额默认 `核销金额`；订单量默认 `核销券数` | 成交分析展示“意向成交”还是“核销” |
| B11 | 小程序：访问、包间点击、下单、投诉、弹窗/卡券点击、营业额 | 当前 9 个平台 Excel 不提供 | 二期接小程序埋点、订单、投诉、活动点击数据 | 小程序是否纳入一期；若纳入需补样本和字段说明 |
...
| 标记 | demo 模块/字段 | 当前 Excel 支撑度 | 一期可用口径 | 需要客户确认/补数 |
| --- | --- | --- | --- | --- |
| U1 | 用户总数、新客、熟客、老客 | 弱支撑 | 抖音商品有 `成交新客数`，但美团本批无新/熟/老客字段；不建议用评价昵称反推用户 | 客户是否接受“各平台原始用户定义”；需要订单级匿名用户 ID |
| U2 | 流失人数、流失率、复购人数、复购率 | 不支撑 | 平台 Excel 无用户历史消费序列，不能严肃计算流失/复购 | 需订单 ID、用户匿名 ID、最近消费日期、历史消费次数 |
| U3 | 下单周期 | 不支撑 | 需要同一用户多次下单间隔；当前只有日/商品/门店聚合 | 需用户级订单流水 |
| U4 | 消费水平分布 | 部分支撑 | 可用门店/商品客单价做门店层粗分；不能得到用户消费水平分布 | 需用户累计消费、单次订单金额 |
| U5 | 优惠敏感度 | 部分支撑 | 可从商品类型、商品名称、售价/划线价、团购/套餐字段做商品层弱分类 | 需订单级优惠、券、原价支付记录 |
| U6 | 包型消费偏好 | 部分支撑 | 可从美团/抖音商品名称解析 `特惠/中/大/尊/4H/12H` 等标签，做商品或订单弱口径 | 需标准包型字段，避免靠商品名解析 |
| U7 | 下单时段 | 不支撑 | 当前 Excel 没有小时级下单时间 | 需订单创建时间、预约开始时间、消费开始/结束时间 |
| U8 | 工作日/周末、同比、环比筛选 | 支撑查询层 | 工作日/周末由日历派生；同比/环比由查询层计算 | 节假日是否独立分组 |
...
| 标记 | demo 模块/字段 | 当前 Excel 支撑度 | 一期可用口径 | 需要客户确认/补数 |
| --- | --- | --- | --- | --- |
| M1 | 风险总览：总风险项、严重、预警、关注、健康门店占比 | 依赖规则结果 | 基于下方规则触发结果聚合；不是 Excel 原始字段 | 阈值、风险等级、健康门店定义 |
| M2 | 经营业绩：营业额环比、同店增长 | 支撑 | 美团/抖音交易核销金额按门店和日期范围比较 | 同店增长比较周期和门店范围 |
| M3 | 经营业绩：翻台率、包间使用率、单包间产值 | 不支撑 | 当前平台 Excel 无包间和场次；只能标待接入 | 需房态、包间数、场次、营业时长 |
| M4 | 收入核查：退款异常率 | 支撑 | 美团交易退款金额/券数，抖音意向退款金额/券数 | 阈值按金额、券数还是订单数 |
| M5 | 收入核查：0 元体验订单占比、店主下单订单占比 | 不支撑 | 当前 Excel 无订单实付明细和下单人身份 | 需订单级支付金额、用户身份、员工/店主标识 |
| M6 | 流量数据：曝光下滑、星级评分、商圈排名/榜单 | 部分支撑 | 曝光用美团客流/抖音商品；星级用评价明细弱口径；排名用榜单 Excel | 经营分、牌级当前无导出字段 |
| M7 | 用户服务：差评率、评价未回复、客诉响应 | 部分支撑 | 差评率用评价汇总/明细；评价回复状态可用明细；投诉率和客诉时长需投诉系统 | 投诉和评价是否合并为用户服务风险 |
| M8 | 竞争环境：周边竞品、价格敏感度、竞品差评提及 | 部分支撑 | 商圈排名可用榜单；差评提及竞品可用评价文本 NLP；价格敏感度需竞品价 | 竞品清单和竞品价格来源 |
| M9 | 设备、运营执行、合规、回本周期 | 不支撑 | 只能预留规则分类，不纳入一期 Excel 看板 | 需设备档案/工单、督导评分、财务/保险/税务、投资回本数据 |
...
| 标记 | demo 模块/字段 | 当前 Excel 支撑度 | 一期可用口径 | 需要客户确认/补数 |
| --- | --- | --- | --- | --- |
| S1 | 单店筛选与顶部卡片：营业额、客单价 | 支撑 | 营业额默认核销金额；客单价 `核销金额 / 核销券数` | 单店页是否只看选中门店，是否合并平台 |
| S2 | 顶部卡片：场次数、续单率、单包间产值、翻台率 | 不支撑 | 当前平台 Excel 无场次、续单、包间 | 需 POS/小程序订单、房态、包间数、续单标识 |
| S3 | 用户下单行为：消费时段、闲时/高峰、预订习惯、消费时长 | 不支撑 | 平台 Excel 无小时、预约提前量、消费开始/结束时间 | 需订单创建时间、预约时间、开台/离店时间 |
| S4 | 消费渠道偏好 | 部分支撑 | 美团/点评/抖音可按平台交易金额/券数占比；高德/快手/小程序当前无样本 | 是否把点评与美团合并；是否补高德/快手/小程序 |
| S5 | 包型偏好、包型趋势、包型 x 时段热力图 | 部分支撑 | 包型可从商品名称解析；趋势按日期聚合；时段热力图不能做 | 需标准包型、订单时段、房型字段 |
| S6 | 不同包型客单价、换房行为 | 部分/不支撑 | 包型客单价可用商品交易弱算；换房行为不支撑 | 需订单包间、换房事件、前后包型 |
| S7 | 消费水平、时段客单价、时长 x 客单价 | 部分/不支撑 | 消费水平可用订单/商品金额弱分布；时段和时长相关不支撑 | 需订单级金额、时间、消费时长 |
| S8 | 价格敏感度、消费能力客群分层 | 部分/不支撑 | 可用团购/套餐/原价商品弱分类；客群分层需用户累计消费 | 需优惠明细、用户 ID、累计消费 |
| S9 | 新老用户、复购率、消费频次、生命周期 | 不支撑 | 当前 Excel 无用户历史序列 | 需用户匿名 ID、订单历史 |
| S10 | 流失用户、沉默用户分析 | 不支撑 | 当前 Excel 不能识别用户最近一次消费 | 需用户最近消费日期和召回规则 |
| S11 | 用户画像标签、筛选命中、用户列表 | 不支撑 | 只能作为二期标签系统设计，不应一期伪造 | 需会员/订单/渠道/触达授权 |
| S12 | 运营动作：推送、企微、导出名单、创建营销活动 | 不支撑 | 属于营销系统能力，不是平台 Excel 字段 | 需触达系统、权限、合规规则 |

### 3.7 用户分析

本批平台 Excel 不包含稳定用户 ID、订单 ID、包间、消费开始/结束时间，因此不能支撑完整用户画像。可先做弱口径：

| demo 指标 | 一期可做 | 一期口径 | 二期需要 |
| --- | --- | --- | --- |
| 新客 | 部分 | 抖音商品 `成交新客数`；美团本批无新客字段 | 用户级订单 |
| 熟客/老客 | 否 | 不建议反推 | 用户历史消费次数 |
| 流失/复购 | 否 | 不建议反推 | 用户最近消费日期 |
| 消费水平 | 部分 | 商品/门店客单价粗分 | 用户累计消费 |
| 优惠敏感度 | 部分 | 商品类型、售价、原价、下单/核销金额差额 | 订单级优惠 |
| 包型偏好 | 部分 | 从商品名称解析 `特惠 / 中包 / 大包 / 尊享 / 12H / 4H` 等标签 | 标准房型字段 |
| 下单时段 | 否 | 平台汇总没有小时 | 订单创建时间 |

### 3.8 异常监控

| 告警项 | 一期是否可做 | 口径 |
| --- | --- | --- |
| 营业额环比/同比 | 是 | 核销金额按门店、平台、日期范围比较 |
| 曝光下滑 | 是 | 美团客流 `曝光人数` 环比；抖音商品曝光按品牌/商品 |
| 访问转化下降 | 是 | 美团 `曝光访问转化率`、`意向转化率` |
| 退款异常 | 是 | 退款金额 / 下单金额，退款券数 / 下单券数 |
| 差评率 | 是 | 美团 `新增差评数 / 新增评价数`；抖音按推荐等级 |
| 评价未回复 | 是 | 评价明细回复状态 |
| 榜单下降 | 是 | 城市/行政区/商圈排名环比 |
| 经营分、牌级 | 暂不可做 | 本批 Excel 未提供 |
| 翻台率、包间使用率 | 不可做 | 需房态/订单 |
| 设备、合规、回本 | 不可做 | 需外部系统 |
